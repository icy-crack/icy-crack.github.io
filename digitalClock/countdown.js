...
function update(){
    ...
    if(nextSeconds != curSeconds){
        ...
        curShowTimeSeconds = nextShowTimeSeconds;   //当秒数变化时改变时钟显示时间
    }
    updateBalls();
...
function addBalls(x, y, num){
    for (var i=0; i<digit[num].length; i++){
        for(var j=0; j<digit[num][i].length; j++){
            if(digit[num][i][j] == 1){    //在时钟点阵图位置添加一个小球
                var aBall = {
...
function render(cxt){
    ...
    renderDigit(MARGIN_LEFT, MARGIN_TOP, parseInt(hours/10), cxt);   //具体位置绘制数字
